...
export const ModifiedFileMessage: React.FC<ModifiedFileMessageProps> = ({
  ...
  const normalizedData = useMemo(() => {
    if (isModifiedFileMessageValue(data)) {
      ...
      return {
        path: typeof data.originalUri === "string" ? data.originalUri : data.originalUri.fsPath,
        isNew: false,
        diff: data.diff || "",
        status: data.state === "applied" ? "applied" as const : data.state === "discarded" ? "rejected" as const : null,
        content: data.content || "",
        messageToken: data.messageToken || "",
