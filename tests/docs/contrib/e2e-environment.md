...
| CORE_VSIX_FILE_PATH    | (Optional) Path to the core extension in vsix format. <br/>If this and CORE_VSIX_DOWNLOAD_URL are not set, the tests will run using the local extension under development (launches VS Code with `--extensionDevelopmentPath` and `--enable-proposed-api`).     |
| JAVA_VSIX_FILE_PATH    | (Optional) Path to the Java extension in vsix format.                                                                                                                                                                                                           |
| CORE_VSIX_DOWNLOAD_URL | (Optional) URL to get vsix from (only used if VSIX_FILE_PATH is not defined)                                                                                                                                                                                    |
