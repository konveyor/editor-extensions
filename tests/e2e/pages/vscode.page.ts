...
export abstract class VSCode {
  ...
  public async startServer(): Promise<void> {
    ...
    try {
      ...
      // Log status before clicking
      const statusBefore = await analysisView
  ...
  public async startServerViaCommand(maxRetries: number = 10): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.log(
          `[Attempt ${attempt}] Waiting 5 minutes for language providers to initialize...`
        );
        ...
        console.log(
          `[Attempt ${attempt}] Executing command: ${VSCode.COMMAND_CATEGORY}: Start Server`
        );
        ...
        if (hasNoProvidersError) {
          ...
          if (attempt < maxRetries) {
            ...
            // Last attempt failed
            throw new Error(
              `Server failed to start after ${maxRetries} attempts (${maxRetries * 5} minutes total wait time). Language providers never became ready.`
            );
          }
        }
        ...
        try {
          ...
          console.log(
            `Server started successfully after ${totalWaitMinutes} minutes total wait time`
          );
          ...
          if (attempt < maxRetries) {
            ...
            throw new Error(
              `Server failed to start after ${maxRetries} attempts. Stop button never appeared.`
            );
          }
        }
      } catch (error) {
        ...
        if (attempt >= maxRetries) {
          ...
          throw new Error(
            `Server failed to start after ${maxRetries} attempts (${maxRetries * 5} minutes total). Last error: ${error instanceof Error ? error.message : String(error)}`
          );
        }
        // Otherwise, retry
  ...
  public async listInstalledExtensions(): Promise<void> {
    try {
      console.log('=== Listing Installed Extensions ===');

      // Method 1: Use VSCode API if available
      try {
        const extensions = await this.window.evaluate(() => {
          const vscode = (window as any).vscode;
          if (!vscode?.extensions) return null;

          return vscode.extensions.all
            .filter((ext: any) => !ext.id.startsWith('vscode.')) // Filter out built-in extensions
            .map((ext: any) => ({
              id: ext.id,
        ...
        if (extensions && extensions.length > 0) {
          ...
          extensions.forEach((ext: any) => {
            ...
            console.log(`${status} - ${ext.id} (v${ext.version || 'unknown'})`);
          });
          console.log('');
          return;
        }
      } catch (error) {
        console.log('VSCode API not available, trying command palette...');
      }

      // Method 2: Use Extensions view
      await this.executeQuickCommand('Extensions: Show Installed Extensions');
      await this.window.waitForTimeout(2000);

      // Get extension list from the sidebar
      const extensionItems = await this.window
        .locator('.extensions-list .extension-list-item')
        .all();
      console.log(`Found ${extensionItems.length} extensions:\n`);
      ...
      for (const item of extensionItems) {
        ...
        const id = await item
          ...
          .textContent()
          .catch(() => '');
        const version = await item
          .locator('.version')
          .textContent()
          .catch(() => '');

        console.log(`- ${name} ${id ? `(${id})` : ''} ${version}`);
      }

      console.log('\n=== End Extensions List ===\n');
    } catch (error) {
      console.error('Failed to list extensions:', error.message);
    }
  }
}
