...
providers.forEach((config) => {
  test.describe(
    `Coolstore app tests with agent mode enabled - offline (cached) | ${config.provider}/${config.model}`,
    { tag: ['@tier0', '@offline'] },
    () => {
      ...
      test('Fix JMS Topic issue with agent mode enabled (offline)', async () => {
        ...
        while (!done) {
          ...
          if (
            (await resolutionView.getByText('Done addressing all issues. Goodbye!').count()) > 0
          ) {
          ...
          const acceptChangesLocator = resolutionView
