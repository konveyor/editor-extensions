...
test.describe('C# Extension - Installation & Startup', { tag: '@tier0' }, () => {
  ...
  test('Can access analysis view after opening it', async () => {
    ...
    console.log(`Error dialogs present: ${hasErrorDialog}`);

    // For C# extension, verify the analysis view tab is still accessible
    ...
    // Verify the analysis view content is loaded
    const analysisView = await vscodeApp.getView(KAIViews.analysisView);
    const pageComponent = analysisView.locator('[class*="pf-v"][class*="-c-page"]').first();
    const isViewLoaded = await pageComponent.isVisible().catch(() => false);
    console.log(`Analysis View loaded: ${isViewLoaded}`);
    ...
    console.log(
      `✅ All notifications clear (${errorCount} errors found) - C# extension is installed and activated`
    );
