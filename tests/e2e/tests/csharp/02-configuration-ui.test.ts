...
test.describe.serial('C# Extension - Configuration & UI', { tag: '@tier0' }, () => {
  let vscodeApp: VSCode;
  const randomString = generateRandomString();
  const profileName = `csharp-automation-${randomString}`;
  let repoInfo: RepoData[string];
  const screenshotDir = pathlib.join(SCREENSHOTS_FOLDER, 'csharp-configuration-ui');
  ...
  test('Create Profile with custom rules for C#', async () => {
    await vscodeApp.waitDefault();
    // Create profile with custom rules (dotnet-core-migration rulesets)
    // For C#, we use custom rules folder instead of predefined targets
