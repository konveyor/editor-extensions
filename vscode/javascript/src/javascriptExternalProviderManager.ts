 ...
 */
export class JavaScriptExternalProviderManager implements vscode.Disposable {
  ...
  constructor(
    ...
    this.logger = logger.child({ component: "JavascriptExternalProviderManager" });
  }
  ...
  async start(): Promise<void> {
    if (this.process) {
      this.logger.info("javascript-external-provider already running");
      return;
    }
    ...
    if (!fs.existsSync(binaryPath)) {
      const message = `javascript-external-provider binary not found at ${binaryPath}`;
      this.logger.error(message);
      throw new Error(message);
    }

    this.logger.info(`Starting javascript-external-provider`, {
      binaryPath,
      socket: this.providerSocketPath,
    });

    // Spawn the provider process
    this.process = spawn(binaryPath, ["-name", "nodejs", "-socket", this.providerSocketPath], {
    ...
    this.process.stdout.on("data", (data) => {
      ...
      this.logger.info(`[javascript-external-provider] ${message}`);
    });

    // Log stderr
    this.process.stderr.on("data", (data) => {
      const message = data.toString().trimEnd();
      this.logger.error(`[javascript-external-provider] ${message}`);
    });

    // Handle process exit
    this.process.on("exit", (code, signal) => {
      this.logger.info(`javascript-external-provider exited`, { code, signal });

      if (!this.disposed && code !== 0) {
        this.logger.error(`javascript-external-provider crashed with code ${code}`);
        vscode.window.showErrorMessage(
          `Javascript external provider crashed (code: ${code}). Please check the logs.`,
        );
      }
    ...
    this.logger.info(`javascript-external-provider started with pid ${this.process.pid}`);
  }
  ...
  dispose(): void {
    ...
    if (this.process) {
      this.logger.info("Stopping javascript-external-provider");
      this.process.kill();
      this.process = null;
    }
  }
