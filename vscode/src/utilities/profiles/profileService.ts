...
// --- Helpers to resolve config from profile ---
...
// async function persistProfileUpdate(state: ExtensionState, profile: AnalysisProfile) {
//   const allUserProfiles = getUserProfiles(state.extensionContext);
//   const updated = allUserProfiles.map((p) => (p.id === profile.id ? profile : p));
//   await saveUserProfiles(state.extensionContext, updated);

//   state.mutateData((draft) => {
//     const existing = draft.profiles.find((p) => p.id === profile.id);
